...
// Some card names contain an apostrophe (e.g. "Lil' Buddy"). Several image
// tags below build their png->webp fallback as an inline
// onerror="...this.src='card_images/${name}.webp'" attribute. If `name`
// contains a literal apostrophe, it terminates that single-quoted JS string
// early and throws "Uncaught SyntaxError: Unexpected identifier", breaking
// the page. URL-encoding the apostrophe keeps the string valid JS while
// still resolving to the exact same file (browsers decode %27 back to ' when
// fetching), so this is safe to use anywhere a name is interpolated inside
// a single-quoted src assignment.
function urlSafeCardName(name) {
    return String(name || '').replace(/'/g, '%27');
}

// card_data.json now carries an authoritative "Image" field on every card,
// generated by matching each card's Name/OldName against the real files
// that exist in card_images/ on disk (fixing typos, renames, and
// hyphen/underscore/apostrophe mismatches once, at data-build time, instead
// of guessing filenames and chaining png->webp->alias fallbacks at render
// time). `name` can be a database key ("Doom-Shroom") or a display name
// with spaces ("Doom Shroom") - both resolve to the same card via
// cardDatabase's alias Proxy.
function getCardImageSrc(name) {
    ...
    // No mapped file (a small handful of cards have no art on disk at all).
    // Fall back to the old best-guess filename so this never behaves worse
    // than before for anything unexpected.
    return `card_images/${urlSafeCardName(key.replace(/ /g, '_'))}.png`;
}

// Some cards were renamed at some point (e.g. "Fig" -> "Transfiguration").
// Old saved decks and old image filenames may still reference the former
// name. Rather than injecting extra keys straight into cardDatabase (which
// made Object.keys(cardDatabase) return 2-3 entries for the same card —
// showing up as duplicates in the collection grid, search, and even
// skewing pack-opening odds toward renamed cards), wrap the raw data in a
// Proxy: cardDatabase['Doom_Shroom'] still resolves via OldName, but
// Object.keys/values/entries only ever see the real, canonical keys.
function makeAliasedCardDatabase(rawData) {
    ...
    return new Proxy(rawData, {
        ...
        // ownKeys/getOwnPropertyDescriptor intentionally left untouched, so
        // Object.keys/values/entries/for-in only ever iterate real cards.
    });
}
    ...
    htmlString += `
        <div class="rec-row starter-rec-row">
            <div class="rec-card starter-rec-card" style="--d: 0ms;">
                ...
                <img
                    src="${getCardImageSrc(randomCard)}"
                    alt="${escapeHtml(displayName)}"
                    title="${escapeHtml(displayName)}"
                    onerror="this.onerror=null; this.src='card_images/${urlSafeCardName(randomCard)}.webp';"
                >
                ...
                <div class="swap-duel">
                    ...
                    <div class="swap-duel-cards">
                        <div class="swap-card out">
                            <img src="${getCardImageSrc(bestSwapIdea.removeCard)}" alt="${weakName}" title="${weakName}"
                                onerror="this.onerror=null; this.src='card_images/${urlSafeCardName(bestSwapIdea.removeCard)}.webp';">
                            <span class="swap-tag">Out</span>
                        </div>
                        <span class="swap-arrow">➜</span>
                        <div class="swap-card in">
                            <img src="${getCardImageSrc(bestSwapIdea.addCard)}" alt="${topName}" title="${topName}"
                                onerror="this.onerror=null; this.src='card_images/${urlSafeCardName(bestSwapIdea.addCard)}.webp';">
